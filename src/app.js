...
if(user.db){
    ...
    const db=user.db
  return  res.json({respose:true , db:db})
...
app.post("/crearinstanciadb",async (req,res)=>{
  const {db,user,contrasena}=req.body
   ...
   const {sequelize,usuarioaliasalmacen, vendedor, almacen ,usuario} = crearConexionPorNombre(db);
  ...
  if (!usuario) {
    ...
    if(usuario.db){
      if(data.metodo==="traeralmacen"){
        socket.emit("obteneralmacen",{almacen:socket.request.session.usuario.almacen})
      }
    }else{
